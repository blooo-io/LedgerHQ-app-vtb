...
parser_error_t parser_getNumItems(const parser_context_t *ctx, uint8_t *num_items) {
    ...
    if (!parser_show_expert_fields()) {
        ...
        for (uint8_t argIdx = 0; argIdx < methodArgCount; argIdx++) {
            bool isArgExpert = _getMethod_ItemIsExpert(ctx->tx_obj->callIndex.moduleIdx,
...
parser_error_t parser_getItem(const parser_context_t *ctx,
    ...
    *pageCount = 1;
    ...
    if (!parser_show_expert_fields()) {
        ...
        while ((argIdx < methodArgCount) && _getMethod_ItemIsExpert(ctx->tx_obj->callIndex.moduleIdx,
                                                                    ctx->tx_obj->callIndex.idx, argIdx)) {
            argIdx++;
            displayIdx++;
        }
